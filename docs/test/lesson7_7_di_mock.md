...
**🐘ガネーシャ：** 「ポイントはな、**ログに何も出力されてない**ことや。本物の NotificationService は動いてないんや」
...
**🐘ガネーシャ：** 「ほな、本当に Mock が機能してるか確認してみよか。ログファイルを開いてみ」

```
📁 storage/logs/laravel-YYYY-MM-DD.log
```

**💡 ヒント：** ログファイルは日付ごとに分かれてるで。今日の日付のファイルを開いて、`[Notification]` で検索してみてな。

**👩‍💻ユーザー：** 「あれ？テストを2つとも実行したのに、`[Notification]` のログが1つも出てない...」

**🐘ガネーシャ：** 「**それが正解や！** Mock が正しく機能してる証拠やで」
