...
**🐘ガネーシャ：** 「Interface を使っても、テストはちゃんと動くやろ？ほな、ログファイルを開いて `[Notification]` で検索してみ」

**👩‍💻ユーザー：** 「ログが1つも出てないですね！Mock がちゃんと機能してる証拠ですね！」

**🐘ガネーシャ：** 「**その通りや！** `instance()` で Mock を登録したから、`bind()` より優先されて偽物が使われたんや」
...
│                                                             │
│  【なぜ？】                                                 │
│  → instance() > bind() の優先順位で Mock が使われた        │
│  → 本物の LogNotificationService は動いてない             │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```

**👩‍💻ユーザー：** 「Interface を使っても、テストでは Mock がちゃんと使われるんですね！」
...
**🐘ガネーシャ：** 「**Interface の場合は話が違う**んや。第1章で説明したやろ？Interface は**『契約書』**や。『notify() というメソッドがある』っていうルールだけ決めてて、**中身の実装は書いてない**んや」

**👩‍💻ユーザー：** 「あ、そうでした！『何ができるか』だけ定義して、『どうやるか』は書かないんでしたよね」

**🐘ガネーシャ：** 「せや。**中身がないから `new` できへん**んや。`new` っちゅうのは『実際に動くモノを作る』ことやけど、契約書だけあっても動くモノは作れへんやろ？」
...
├─────────────────────────────────────────────────────────────┤
│                                                             │
│  【なぜ Interface は new できないのか？】                   │
│                                                             │
│  Interface = 契約書・ルール                                 │
│  ・「notify() メソッドがある」というルールだけ             │
│  ・実際の処理（ログ出力？メール送信？）は書いてない        │
│  ・中身がないから new しても動くモノが作れない！           │
│                                                             │
│  ─────────────────────────────────────────────────          │
│                                                             │
│  【具体的なクラスとの違い】                                 │
│                                                             │
│  LogNotificationService（具体的なクラス）                   │
│  → 中身がある（Log::info() を呼ぶ処理が書いてある）        │
│  → new できる → Laravel が勝手に作れる                    │
│                                                             │
│  NotificationServiceInterface（Interface）                  │
│  → 中身がない（契約書だけ）                                │
│  → new できない → どれを作ればいいか分からない！         │
│                                                             │
│  ─────────────────────────────────────────────────          │
│                                                             │
...
│  → Laravel「Interface は契約書だけで中身がない...」       │
│  → Laravel「new できへん...」                             │
│  → Laravel「LogNotificationService？MailNotificationService？」│
│  → Laravel「どの具体的なクラスを使えばええんや？」        │
│  → ❌ エラー！                                             │
│                                                             │
│  ─────────────────────────────────────────────────          │
│                                                             │
│  💡 だから AppServiceProvider で                            │
│     「この Interface にはこの具体的なクラスを使え」         │
│     と教えてあげる必要がある！                             │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```

**👩‍💻ユーザー：** 「なるほど！Interface は契約書だから中身がない。中身がないから `new` できない。だから『どの具体的なクラスを使うか』を教えてあげないといけないんですね！」
...
**🐘ガネーシャ：** 「ここで大事な概念を整理しておくで。**密結合**と**疎結合**や」

**👩‍💻ユーザー：** 「密結合...疎結合...？」

**🐘ガネーシャ：** 「簡単に言うと、**どれくらい密接に繋がってるか**っちゅうことや」
...
│                                                             │
│  ┌────────────────┐      ┌────────────────────────┐        │
│  │    UseCase     │ ──→ │  LogNotificationService │        │
│  └────────────────┘      └────────────────────────┘        │
│          │                         │                        │
│          │   具体的なクラスに依存   │                        │
│          └─────────────────────────┘                        │
│                                                             │
│  😱 問題点                                                  │
│  ・LogNotificationService を変えると UseCase も影響受ける  │
│  ・MailNotificationService に変えるには UseCase を書き換え │
│  ・UseCase が「ログ出力する」ことを知ってしまってる        │
│                                                             │
│  例え：「田中さんに電話する」と決め打ちしてる状態          │
│  　　　田中さんが辞めたら、全部書き換えないといけない      │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```
...
├─────────────────────────────────────────────────────────────┤
│                                                             │
│  「相手の契約（Interface）だけ知っている」状態              │
│                                                             │
│  ┌────────────────┐      ┌──────────────────────────────┐  │
│  │    UseCase     │ ──→ │ NotificationServiceInterface │  │
│  └────────────────┘      └──────────────────────────────┘  │
│                                    △                        │
│                                    │ implements             │
│                    ┌───────────────┼───────────────┐        │
│                    │               │               │        │
│                    ▼               ▼               ▼        │
│              LogNotification  MailNotification  Slack...    │
│              Service          Service                       │
│                                                             │
│  ✅ メリット                                                │
│  ・実装クラスを変えても UseCase は影響を受けない           │
│  ・UseCase は「通知できる」ことだけ知ってる（中身は知らない）│
│  ・新しい実装を追加しても UseCase の変更不要               │
│                                                             │
│  例え：「担当者に電話する」という契約だけ決めてる状態      │
│  　　　担当者が田中→鈴木に変わっても、コードは変わらない  │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```

**👩‍💻ユーザー：** 「なるほど！密結合だと相手が変わると自分も変わらないといけない。疎結合だと相手が変わっても自分は変わらなくていいんですね！」

**🐘ガネーシャ：** 「**完璧や！** Interface を挟むことで疎結合になるんや」

---

### 🔄 依存の逆転（Dependency Inversion Principle）

**🐘ガネーシャ：** 「もう一つ大事な概念があるで。**依存の逆転**や。これは SOLID 原則の D にあたるもんや」

**👩‍💻ユーザー：** 「依存の逆転...？依存がひっくり返るってことですか？」

**🐘ガネーシャ：** 「せや！図で見た方が分かりやすいで」

```
┌─────────────────────────────────────────────────────────────┐
│              依存の逆転（DIP: Dependency Inversion Principle）│
├─────────────────────────────────────────────────────────────┤
│                                                             │
│  【Before: 依存の逆転なし（密結合）】                       │
│                                                             │
│  ┌──────────────┐                                           │
│  │   UseCase    │  ← 高レベル（ビジネスロジック）          │
│  └──────┬───────┘                                           │
│         │ 依存                                              │
│         ▼                                                   │
│  ┌──────────────────────┐                                   │
│  │ LogNotificationService│  ← 低レベル（具体的な実装）     │
│  └──────────────────────┘                                   │
│                                                             │
│  😱 問題：高レベルが低レベルに依存してる！                 │
│  　　　　 低レベルを変えると高レベルも影響を受ける         │
│                                                             │
│  ─────────────────────────────────────────────────          │
│                                                             │
│  【After: 依存の逆転あり（疎結合）】                       │
...
│         ▼                                                   │
│  ┌──────────────────────────────┐                           │
│  │ NotificationServiceInterface │  ← 抽象（契約）          │
│  └──────────────────────────────┘                           │
│         △                                                   │
│         │ 依存（implements）                                │
│         │                                                   │
...
│  └──────────────────────┘                                   │
│                                                             │
│  ✅ ポイント：低レベルが抽象（Interface）に依存するように  │
│  　　　　　　 依存の方向が「逆転」した！                   │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```

**👩‍💻ユーザー：** 「あ！Before では UseCase → LogNotificationService だったのが、After では LogNotificationService → Interface になってる！」

**🐘ガネーシャ：** 「**その通りや！** 矢印の向きが逆転しとるやろ？これが**依存の逆転**や」
...
│  高レベル（UseCase）                                        │
│       ↓ 依存                                                │
│  低レベル（LogNotificationService）                         │
│                                                             │
│  「UseCase が LogNotificationService を使う」               │
│  → UseCase が具体的な実装に依存                            │
│                                                             │
│  ─────────────────────────────────────────────────          │
│                                                             │
...
│  高レベル（UseCase）                                        │
│       ↓ 依存                                                │
│  抽象（NotificationServiceInterface）← UseCase が定義！   │
│       ↑ 依存                                                │
│  低レベル（LogNotificationService）                         │
│                                                             │
│  「LogNotificationService が Interface に従う」             │
│  → 低レベルが抽象（UseCase が求める契約）に依存           │
│                                                             │
│  ─────────────────────────────────────────────────          │
│                                                             │
│  💡 つまり...                                               │
│  ・従来：UseCase が実装に合わせる                          │
│  ・逆転後：実装が UseCase の契約に合わせる                 │
│                                                             │
│  主導権が「実装」から「UseCase（ビジネスロジック）」へ移動│
│                                                             │
└─────────────────────────────────────────────────────────────┘
```

**👩‍💻ユーザー：** 「なるほど！UseCase が『notify() を持ってる何かが欲しい』って言って、実装側がその契約に従うようになるんですね！」

**🐘ガネーシャ：** 「**完璧や！** これで UseCase は具体的な実装を知らんでも動くし、実装を自由に差し替えられる。これが Interface を使った DI の本質なんや」
...
│  ・どちらも「抽象（Interface）」に依存すべき              │
│                                                             │
│  【今回の例で言うと】                                       │
│  ・高レベル = UseCase（ビジネスロジック）                  │
│  ・低レベル = LogNotificationService（具体的な通知処理）   │
│  ・抽象 = NotificationServiceInterface（契約）             │
│                                                             │
│  【得られるメリット】                                       │
│  ✅ UseCase は具体的な実装を知らなくていい                 │
│  ✅ 実装を自由に差し替えられる（ログ→メール→Slack）      │
│  ✅ テストで Mock に差し替えやすい                         │
│  ✅ 変更に強い（片方を変えてももう片方に影響しない）       │
│                                                             │
│  ─────────────────────────────────────────────────          │
│                                                             │
│  💡 Interface を挟む = 依存の逆転を実現する手段            │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```

**🐘ガネーシャ：** 「これで密結合・疎結合・依存の逆転、全部理解できたな！」

**👩‍💻ユーザー：** 「はい！Interface って単なる契約書じゃなくて、依存関係を逆転させて疎結合を実現するための仕組みなんですね！」

**🐘ガネーシャ：** 「**さすがワシの弟子や！** 完璧に理解しとる。ワシの教え子のボブおじさんも喜ぶで」

**👩‍💻ユーザー：** 「ボブおじさん...？（SOLID 原則を提唱したロバート・C・マーティンのことかな...？）」
...
| 密結合（Tight Coupling） | 具体的な実装に直接依存。変更の影響を受けやすい |
| 疎結合（Loose Coupling） | 抽象（Interface）に依存。変更に強い |
| 依存の逆転（DIP） | 低レベルが抽象に依存するようにする原則 |
| bind() | Interface と実装クラスを紐づける |
| instance() | 特定のオブジェクトを登録する（bind より優先） |
| Mailtrap | 開発・テスト用のダミーメールサーバー |
| YAGNI | 必要になるまで作るな |
| SOLID原則 | 良い設計の5原則（S,O,L,I,D）。DIP は D にあたる |
