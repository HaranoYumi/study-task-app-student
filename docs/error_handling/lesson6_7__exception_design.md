...
**🐘ガネーシャ：** 「1つ目、**ApiResponse**。レスポンス形式を統一するクラスや。これを使えば、誰が書いても同じ形式になる」

**👩‍💻ユーザー：** 「なるほど」

**🐘ガネーシャ：** 「2つ目、**ApiExceptionHandler**。例外を HTTP レスポンスに変換するクラスや。例外の種類に応じて、適切なメッセージを返してくれる」

**👩‍💻ユーザー：** 「例外を一括で管理する...」

**🐘ガネーシャ：** 「3つ目、**request_id**。各リクエストに一意のIDを付けて、ログと照合できるようにする」
...
## 🔍 第3章：ApiExceptionHandler で例外を一括管理
...
## 🔍 第4章：request_id でエラーを追跡可能に

### 🎭 問題の再確認

**🐘ガネーシャ：** 「3つ目の問題、『本番でエラー調査ができない』を解決するで」
...
**👩‍💻ユーザー：** 「`uniqid` で一意のIDを作るんですね」

**🐘ガネーシャ：** 「せや。`req_` っていう接頭辞を付けて、一目でリクエストIDって分かるようにしとるんや」
...
**🐘ガネーシャ：** 「UseCase はこんな感じや。Lesson 8, 9 で学んだ内容やな」
...
**🐘ガネーシャ：** 「せや！Lesson 8 で学んだ通りや。例外は上に伝播させて、ApiExceptionHandler に任せるんや」
...
│  └── 403, 409 を返す場面                                   │
│                                                             │
│  Lesson 7: try-catch の基本                                 │
│  └── 例外をキャッチして処理する                            │
│                                                             │
│  Lesson 8: UseCase での throw                               │
│  └── 例外は throw だけ、try-catch は書かない               │
│                                                             │
│  Lesson 9: トランザクション                                 │
│  └── 複数テーブル更新を安全に                              │
│                                                             │
│  Lesson 10: 例外処理の全体設計（今日）                      │
...
| 9 | トランザクション | DB::transaction() で複数更新を安全に |
| **10** | **例外処理の全体設計** | **ApiResponse + ApiExceptionHandler + request_id** |
| 11（次回） | カスタム例外 | 409 を返せる ConflictException の作成 |
